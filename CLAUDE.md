...
PDFs generated by `/run` and `/combine-runs` are uploaded to Google Drive after generation:

- **Drive folder ID**: `1461BCTukC-zQY2yAZTtEIRcqraZRrnqU`
- **Drive URL**: https://drive.google.com/drive/folders/1461BCTukC-zQY2yAZTtEIRcqraZRrnqU
- **Workspace**: `personal` (via `mcp__jungle-personal__gws-personal__upload_file`)
- **Organization**: Each run slug gets its own subfolder inside the Drive folder. Combined PDFs go under `combined--<slug>`.
